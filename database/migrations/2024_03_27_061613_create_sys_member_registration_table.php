...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('sys_member_registration', function (Blueprint $table) {
            $table->comment('Data semua member');
            $table->increments('member_registration_id');
            $table->string('member_name', 100)->default('')->comment('Nama Member');
            $table->string('member_email', 30)->default('')->index('member_email')->comment('Email Member');
            ...
            $table->string('member_devisor_mobilephone', 16)->default('')->comment('Nomor Kontak Pewaris Member');
            $table->string('member_registration_username', 50);
            $table->text('member_registration_password');
            $table->string('member_registration_sponsor_username', 50);
            $table->integer('member_registration_product_package_id')->default(0);
            $table->dateTime('member_registration_datetime');
            $table->enum('member_registration_status', ['requested', 'approved', 'rejected'])->default('requested');
            $table->dateTime('member_registration_status_datetime');
            $table->enum('member_registration_transaction_type', ['warehouse', 'stockist'])->default('warehouse');
            $table->integer('member_registration_transaction_id')->default(0)->comment('referensi id transaksi');
            $table->unsignedInteger('member_id');
            $table->boolean('member_is_network')->default(false);
            $table->string('member_network_slug');
            $table->string('member_job', 100);
        });
    }
    ...
    {
        Schema::dropIfExists('sys_member_registration');
    }
};
